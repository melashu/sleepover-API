        ...
        get 'Retrieves all Reservations' do
          ...
          response '200', 'reservations retrieved' do
            ...
            before do
              ...
              @reservation = Reservation.create([{ room_id: @room.id, user_id: @admin.id, start_date: time.now, end_date: time.now + 30000, archived: true}])
            end
            schema type: :array,
             items: {
               type: :object,
      ...
      path '/api/v1/reservations' do
               ...
               type: :object,
					...
					response '201', 'resrevation added' do
						...
						let(:reservation) do
							{ room_id: @room.id, user_id: @admin.id, start_date: time.now, end_date: time.now + 30000, archived: true}
						end
					end
